...
clc
dbstop if error
subjs = {'540','544','552','567','584','596'};
sex = {'M','M','M','F','M','M'};
set = 'Training'; %'Training', 'Testing'
...
for su = 1:length(subjs)
    ...
    if(isfield(struct.patient.sleep,'event'))
        ...
        patient.timeseries.sleep.time_begin(1) = datetime('','InputFormat','dd-MM-uuuu HH:mm:ss');
        patient.timeseries.sleep.time_end(1) = datetime('','InputFormat','dd-MM-uuuu HH:mm:ss');
        patient.timeseries.sleep.quality(1) = nan;
    end
    disp('- OK. (07/17)');
    ...
    disp('- Parsing Acceleration data...');
    acceleration = struct.patient.acceleration.event;
    if(length(acceleration)==1)
        patient.timeseries.acceleration.time(1) = datetime(struct.patient.acceleration.event(1).Attributes.ts,'InputFormat','dd-MM-uuuu HH:mm:ss');
        patient.timeseries.acceleration.value(1) = str2double(struct.patient.acceleration.event(1).Attributes.value);
    else
        for b = 1:length(acceleration)
            patient.timeseries.acceleration.time(b) = datetime(struct.patient.acceleration.event{b}.Attributes.ts,'InputFormat','dd-MM-uuuu HH:mm:ss');
            patient.timeseries.acceleration.value(b) = str2double(struct.patient.acceleration.event{b}.Attributes.value);
        end
    end
    disp('- OK. (17/17)');
